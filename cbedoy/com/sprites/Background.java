...
public class Background {
	 ...
	 *
	 */
	public Background(StageViewController s, String imgName) {
		this(s, imgName, 1.0, 0, IMG_DIMENSIONS, IMG_DIMENSIONS, 1.0F);
	}

	public Background(StageViewController s, String imgName, double speedFactor,
					double defaultSpeed, int width, int height,
					float alpha) {
		this.stageViewController = s;
		this.map = ((MainViewController)s).getCurrentMap();
	...
	private void setImage(String imgName) {
		setImage(stageViewController.getImagesLoader().getImage(imgName));
	}
	...
	private void setImage(BufferedImage img) {
		...
		if (img.getWidth() != width	||
			img.getHeight() != height) {
				this.img = stageViewController.getImagesEffects().
	...
	private void paintImg(Graphics g, int x2, int y2) {
		stageViewController.getImagesEffects().paintAlphaImg(
