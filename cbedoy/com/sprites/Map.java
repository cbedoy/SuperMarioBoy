...
import com.sprites.objets.Brick;
import com.sprites.objets.Coin;
import com.sprites.objets.CoinBox;
import com.sprites.objets.Player;
...
public class Map {
	...
	public Map(StageViewController s, String fileName) {
		this.stageViewController = s;
	...
	public Map(StageViewController s, int world, int level) {
	...
	private void readChar(char c, int x, int y) {
		...
		switch (c) {
			case 'B':
				o = new Brick(stageViewController);
				break;
			case 'b':
				o = new Brick(stageViewController);
				...
				break;
			case 'c':
				o = new Coin(stageViewController);
				break;
			case '?':
				o = new CoinBox(stageViewController);
				break;
			case '*':
...
public boolean endlevel = false;
	...
	public Background createBackground(String info) {
		...
		if (st.countTokens() == 1) {
			Background b = new Background(stageViewController, info);
			return b;
		...
		Background b = new Background(stageViewController, name, speedFactor,
	...
	public void startMusic() {
		for (int i=0; i<music.size(); i++) {
			...
			if (songName != null) {
				stageViewController.getSoundsLoader().play(
					music.get(i), true);
			}
		}
	}
